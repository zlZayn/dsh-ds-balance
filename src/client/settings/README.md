...
- 回根 → [../../AGENTS.md](../../AGENTS.md)（src/ 规则层）。

## 文件

### BalanceSettingsCard.tsx

- 职责：卡片本体。持有卡片展开、密钥显隐、四个分组展开四种局部状态；把 `useConfigForm` 的状态翻译成 JSX。
- 关键导出：`BalanceSettingsCard`（`:82`）、`BalanceSettingsCardProps`（`:26`），并转发 `SettingsScope`（`:23`）。
- 分组：连接 → 展示 → 阈值 → 刷新，顺序与宿主 `Config` 的字段顺序一致。
- 默认展开：只有连接组（`DEFAULT_GROUP_OPEN`，`:37-42`）。
- 组内有非法草稿时该组强制展开（`groupOpenNow`，`:96-97`），否则 footer 的「请检查标红的字段」会指向一个收起来的组。
...
- 关键规则：`.card`（`:3`，0.5px 边框 + radius 16）、`.header`（`:21`）、`.body`（`:84`，border-top 0.5px + margin 0 16px + **padding-bottom 0**）、`.footer`（`:99`，padding 12px 0 4px + border-top 0.5px）。
...
- 关键导出：`FieldGroup`（`:47`）、`FieldFrame`（`:137`）、`FieldBadges`（`:93`）、`TextControl`（`:176`）、`SecretControl`（`:210`）、`SelectorControl`（`:264`）、`ActionRow`（`:314`）、`classNames`（`:21`），以及 `FieldStatus`（`:65`）与 `SelectorOption`（`:240`）。
- `FieldGroup` 的折叠头用原语 `DisclosureRow`，不自己画；展开体由原语在 open 时条件渲染，无动画。
- `classNames` 是本目录自备的：包内没有 `clsx` 依赖。
- 被谁依赖：`BalanceSettingsCard.tsx`。
...
- 关键规则：`.group`（`:5`，顶部 12px）、`.groupLast`（`:11`，尾部 12px）、`.groupNote`（`:16`，margin-top 4px）、`.fields > * + *`（`:29`，字段间 0.5px 分隔线）、`.field`（`:34`，padding 12px 0）、`.row`（`:44`，整行选择型，padding 12px 0）、`.input`（`:115`，h34 r8）、`.selector`（`:216`，h36 r18）。
...
- 关键导出：`useConfigForm`（`:388`）、`SettingsScope`（`:24`）、`SettingsScopeSnapshotLike`（`:11`）、`CONFIG_FIELDS`（`:127`）、`SPEC_BY_FIELD`（`:142`）、`FieldState`（`:147`）、`ConfigFormState`（`:165`）、`TestState`（`:179`）、`ConfigFormApi`（`:189`）、`textField` / `numberField` / `selectField`、`currencyCodes`（`:116`）、`AUTO_CURRENCY`（`:105`）、`KNOWN_CURRENCIES`（`:108`）、`probeFailure`（`:222`）、`TEST_LATENCY_MS`（`:211`）。
...
- 只有四个成员：`getSnapshot()`、`subscribe(listener)`、`set(field, value)`、`unset(field)`（`use-config-form.ts:24-33`）。
- 快照只取三片：`value`、`user`、`writable`（`use-config-form.ts:11-19`）。
- 真实 `ctx.settingsScope.bind()` 的快照还带 `status` / `base` / `revision` / `mode`，多余字段不影响结构兼容。
- 适配层在 `src/client/index.tsx:65-83`：它把 `value` 的 `undefined` 收窄成 `{}`、把 `user` 收窄成对象，并在方法缺失时退化成只读。
...
- 因此每个字段写完都要读回：`clear` 要求 `user` 层不再有该字段；`set` 要求 `user` 层有该字段且值相等（`landedWrite`，`use-config-form.ts:288`）。
- 任一字段没落定即整体判失败，草稿保留，footer 出 `role="status"` 提示。
- `set` / `unset` 的返回值必须原样透传（真实实现返回 Promise）；把 Promise 丢掉，读回时永远看不到落定，每次保存都会误报失败。

### 纵向间距只有两个所有者

- `.group` 出顶部 12px（`fields.module.css:5-7`），`.groupLast` 出尾部 12px（`:9-13`）。
- `.body` 的 `padding-bottom` 是 0（`BalanceSettingsCard.module.css:84-90`），卡片 body 不再贡献纵向节奏。
