...
export const zh = {
  ...
  'settings.field.apiKeyRef': 'API Key 引用名',
  ...
  'settings.hint.baseUrl': '形如 https://api.deepseek.com，结尾不要带斜杠。',
  'settings.hint.displayCurrency': '选定币种不存在时，界面会显示账户里实际存在的币种并给出提示。',
  'settings.hint.number': '请填数字；留空表示使用默认值。',
  'settings.hint.threshold': '阈值只在后端生效；界面颜色由后端返回的严重度决定。',
...
export const en: Record<LocaleKey, string> = {
  ...
  'settings.hint.baseUrl': 'For example https://api.deepseek.com, without a trailing slash.',
  'settings.hint.displayCurrency': 'When the chosen currency is missing, the sidebar shows the currency the account actually has and says so.',
  'settings.hint.number': 'Enter a number; leave empty to use the default.',
  'settings.hint.threshold': 'Thresholds apply on the server; the UI colour follows the severity the server returns.',
