...
- `index.tsx`：入口。注册词典、向两个 slot 注册组件。`inject` 是运行时门禁，**只有 `slots` / `locale` 两项**（永远在的服务），删任一项都会让 `apply` 静默不跑；配置表单服务 `configForms` 由 `apply` 内的 `ctx.inject` 把门 —— 缺它只丢卡片，左下角圆环照常。
- `config-slot.ts`：配置表单的**能力探测**（不查版本号）：卡片在这一格拿不拿得到 form。三态 `pending` / `available` / `missing`，**可逆**：槽晚到会把 `missing` 拨回 `available`，已经出现的提示自己撤掉。**探测盯的槽必须与卡片注册的槽是同一个**，否则它会在新宿主上说 available 而卡片其实在别处。纯逻辑 + 可注入时钟，所以能脱离浏览器测；提示文案也在这里。理由见 [决策记录](../../.agents/notes/2026-09-19-capability-probe-for-config-slot.md) 与本轮接缝迁移的记录。
...
- `ENTRY_ID` / `ROW_CONFIG_KEY` / `SIDEBAR_ENTRY_ID`：三个 id 各管一件事 —— 设置命名空间、`plugins.row.config` 的 key（`<包名>#<行 id>`）、左下角条目的 slot id。**它们不是同一个值**，前两个逐字相同只是巧合（包名与行 id 同名）。
- `apply(ctx)`：向 `plugins.row.config` 与 `sidebar.footer.action` 各注册一次；两半共用**同一个** `ConfigForm`（`ctx.configForms.get(ENTRY_ID)`，`apply` 期建一次、引用稳定）—— slot props 里那个 form 与它是同一个对象，所以没有第二条真源。
