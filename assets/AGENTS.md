...
- **首选做法（改进）：元素截图 + `captureBeyondViewport`** —— 对目标元素做 `locator.screenshot()`，让运行时在**自然视口**下把完整高度一次取回来。
  本次实测：配置区高 **1293**、自然视口 **741**，元素截图一次取全、无白边。它**绕开了 `setViewportSize` 把 dpr 压成 1、截图变糊的坑**，所以优于下面那条老路。
  拍完**必须目检**：下半截不是白的、没有被裁；并用**元素自身的几何**（`boundingBox()` 的 `top + height`）确认底部落在取到的范围内。
- **退回老路**：只有在元素截图确实取不全时才临时调高 viewport **高度**（宽度不要动），拍完立刻还原（见 [README.md](README.md)）。
- **（老配方，配合上面那条「退回老路」使用）配置区比详情页的滚动容器高**，容器会把下半截裁掉：直接对配置区做 `locator.screenshot()` 会拍到容器外的页面，
...
- 配置区完整：**插件标题、描述与图标**（插件页按包根 `locale/` 与 `icon.svg` 画）、**四个组名 + 拍摄时点开的四组字段行**、状态徽标、说明行、**唯一的保存按钮**，一样不缺
