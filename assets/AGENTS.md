# assets/ — 规则层

继承根规则，见 [../AGENTS.md](../AGENTS.md)。

只讲**在这里要怎么干**；目录里有什么、被谁引用、图长什么样 → [README.md](README.md)。

## 入口是 bundle 详情页（六张已于第三轮重拍）

配置卡片落在 **bundle 详情页里**：点插件名进详情页，**配置区就在描述下面、直接可改**，
那一行上**没有**第二个 Configure 步骤。落点往返过一次（bundle 页 → 行子页 → bundle 页），
理由见 [决策记录](../.agents/notes/2026-09-22-config-entry-back-to-bundle-config.md)。

- 六张都在**第三轮**（展示元数据 + 图标之后）重拍过；`settings-card*.png` 两张的取景**含**
  图标 / 标题 / 版本 tag / 描述那一块（不只是配置表单），所以那三样在图上可核 —— 见「验收」。
- **进入路径按显示名走**：列表卡片与详情页标题画的是**展示元数据**里的名字（中文「DeepSeek 余额」、
  英文「DeepSeek Balance」），详情页标题下面另有一行代码体写着**完整包名** `dsh-ds-balance`。

  （旧文写的「详情页里的短名是 `ds-balance`」不准确：**没有展示元数据时** Plugins 页回落到的是**完整包名**
  `dsh-ds-balance`；缩掉 `dsh-` 前缀的短名只出现在**设置页**的插件清单里，而且只作用于「回落到包名」那种情况 ——
  有展示元数据时两处显示的都是它。）
- **不需要重启宿主**：本轮改动全在浏览器半边（`src/client/**`），`npm run build` 之后由 `dsh-client-hmr` 换新；
  换不到就刷新一次页面。上一轮那句「重拍要重启一次宿主」是那一轮改宿主半边时的要求，**对本轮不成立**。
- 下面「怎么截」的第 3 步照旧：**Plugins → Installed → 本插件**，滚到配置区完整可见 —— 只是不再有 Configure 那一步（标题改按显示名找）。
- **文件名不许改**：根 README 与 README_en 按名引用，重拍后同名覆盖。

## 什么时候必须重截

判据是「卡片渲染出来的东西有没有变」，不是「改了哪个文件」。
与 [release-guard.mjs](../scripts/release-guard.mjs) 同一种分工：它只判路径，行为等价的改动交给人回答。

必须重截：

- [locales.ts](../src/client/locales.ts) 两份字典的**文案值**（中英任一）
- [BalanceSettingsCard.tsx](../src/client/settings/BalanceSettingsCard.tsx) 的 **JSX 结构**：字段增删、顺序、按钮、状态行
- [fields.tsx](../src/client/settings/fields.tsx) 的**控件形态**：只读行、分组折叠头（`DisclosureRow`）、二级 details
- 三个 `*.module.css` 的**样式值**：字号、行高、间距、令牌、布局
- 卡片**可见状态**的语义：未保存标记、凭据徽标（两态）、只读行留空、失败诊断
- 插件页上的**标题、描述与图标**：真源是包根的 [locale/](../locale/AGENTS.md) 与 [icon.svg](../icon.svg)
  （都是宿主直读，不经我们的代码）。它们画在**列表卡片与详情页两处**，所以动它们要重截 `settings-card*.png` 与
  `settings-cards-position*.png` **四张**
- **版本 tag**：图里那个 `v…` 显示的就是 `package.json` 的版本，所以**重截前先确认版本已经 bump 到即将发布的号**。
  顺序是死的：**先 bump 并提交 → 再拍截图 → 最后发布**，发布流程自己**不** bump
  （判据与理由见 [发布手册](../docs/PUBLISHING.md)；红线「发布流程」钉住 `release.yml` 里不得出现 `npm version`）。
- **`settings-cards-position*.png` 那张（Plugins 列表视图）另有判据**：宿主 Plugins 页的列表结构 —— Official / Installed 两组及其分组文案、本插件在已安装里的可见性（卡片写显示名）、左侧栏收起态。
  已装插件列表变了（多装了一个）**不算过期**：那正是这张图要交代的上下文（「与其他插件并排、长在哪」）。
  **卡片的标题与描述也算它的判据**（列表卡片画的就是这两样，与详情页同源）；
  版本 tag、四组组名、字段与保存按钮归 `settings-card*.png`。
- **左边栏那张另有判据**：[SidebarBalance.tsx](../src/client/sidebar/SidebarBalance.tsx) 的行结构（含**展开态悬浮出的余额气泡**）、
  [BalancePopover.tsx](../src/client/sidebar/BalancePopover.tsx) 的三行与状态行、**标题行右端的 Plugins 图标按钮**、
  以及 [PercentRing.tsx](../src/client/sidebar/PercentRing.tsx) 的**弧长与中心符号** ——
  弧长现在表达比例，余额一变图就变，这是它的正常状态。
  **门面图一律在正常态（币种匹配）下拍**：浮层里没有币种不匹配段、条目上没有感叹号标记；
  若某次确实要在不匹配态下补拍，那一段必须只有一个「改用 X」按钮（出现第二个按钮＝旧版，「去设置」已删）。
  - **两处圆环不是一回事，别为了「看起来一致」去改任何一边**（维护者 2026-09-22 拍板）：
    [icon.svg](../icon.svg) 是**静态品牌环** —— 橙、约七成弧，与余额无关；
    侧栏条目上那条是**实时状态环** —— 颜色由后端 `severity` 决定，余额低于告急线时它就是红的。
    **红环不算判废**（判据只要求币种匹配、无感叹号标记）；门面图里两处颜色不同也是对的。

不必重截：

- 类型与注释（只活在编译期）
- 行为等价的内部重构：抽常量、改名、等价重写
- `README` / `AGENTS` / 测试 / 构建配置
- `inject`、注册 key、`dsh.client.*`：它们坏了是「卡片装不上」，不是「图不准」

拿不准时按「必须重截」办：图的成本低于一张与界面不符的图。

## 完成定义

- 改渲染面的改动，**要么同批带重截图，要么在 commit body 里写明为什么不重截**。
- 图与文案分成两个提交，会留下「文案已改、README 里的图还是旧的」的窗口。

## 怎么截

截图会改**全局设置**（语言等），所以第 0 步是问，不是动手。

0. **先问再动**，问的时候说清三件事，再等维护者确认：
   - 要占用浏览器几分钟；
   - 会临时切界面语言，结束切回；
   - **截图期间请别改语言 / 主题 / 模型等全局设置** —— 它们直接进画面。
1. **用自己的标签页，不动维护者的标签页**：agent 端点新开一个页面打开 GUI 即可，结束时关掉它。
2. **开一个新会话**，再按 [风格基准](README.md#风格基准) 把画面摆到位；摆好后**不再动任何别的东西**。
   - **这一步是隐私要求，不是可选项**：不新开会话，聊天区里就是正在进行的那段对话，
     连工具调用一起进图。拍左边栏那张时尤其致命 —— 它必然带一条聊天区。
   - 新会话的输入框停在**屏幕中央**（不是底部）；为了把它收进画面，取景就得往上延，而上延就会带进侧栏的工作区名与会话标题。
     **隐私优先**：顶端一律压到浮层 / 底部条目那一叠的上沿（第 4 步的例外规则），因此这张很可能不带输入框 —— 那是**接受的代价**，不是缺陷。
3. 进侧边栏 **Plugins → Installed → 本插件那张卡片**（卡片与详情页的标题是**显示名**：中文「DeepSeek 余额」、英文「DeepSeek Balance」；详情页标题下面那行代码体是完整包名 `dsh-ds-balance`。**这一步只服务 `settings-card*.png`** —— 列表视图那张不用点进详情页，见第 6 步），
   滚到配置区完整可见（底部**唯一的保存按钮**必须在图内）。
   四组（连接 / 展示 / 阈值 / 刷新）**默认全部收起** —— **拍摄前依次点开四个组头**，让四组字段行都进画面；
   分组头是 `div[role="button"][aria-expanded]`（默认 `false`），原生的卡片级折叠头与「放弃」按钮都不存在。
   连接组里的二级「自定义设置」**保持收起** —— 那是高级用户的路径，展开它会让图里出现一屏凭据字段。
4. **按快门前收起左侧栏**（隐私硬规则，独立一步）：展开的左侧栏列的是维护者真实的**会话标题与工作区名** —— 那是别人的工作内容，不是装饰，门面图是给外人看的，不能带出去。
   收起**是带动画的**，等固定时长不算保证（本次重拍踩过：zhihu 的 `settings-card_en.png` 一度就是收到一半的中间帧）。
   点收起后**轮询到几何稳定再按快门**：侧栏容器的 `getBoundingClientRect().width` **连续两次采样相同**（间隔约 100ms）、已出现展开按钮（`Open sidebar` / 中文等价）、且宽度落在收起态区间；
   轮询给上限（约 5 秒），**超时按报错处理、重来一次，不许拿中间帧凑数**。
   同类动作一律照此办理：**切语言、切视图、展开或收起折叠区（含二级「自定义设置」与浮层弹出）**之后也要轮询到稳定，别用固定等待当保证。
   收起状态下点不到 `Plugins`，所以顺序是死的：**展开导航 → 走到目标画面 → 收起侧栏 → 按快门**，不能图省事一开始就收。
   **唯一例外是左边栏那两张 `sidebar-popover*.png`**：主体就是侧栏底部条目与浮层，收起侧栏等于删主体 ——
   它改为**把裁剪顶端收到浮层 / 底部条目那一叠的上沿**，把上方的工作区名与会话标题整段剔出去；**隐私优先于输入框 —— 宁可不要输入框，也不能露标题**。
5. **开关状态照现状拍**：拨它会带出「未保存」标记，真保存又改了维护者的真实配置。
6. 界面语言**当前是 English**：先拍英文版（`settings-card_en.png`），再临时切成中文拍 `settings-card.png`，最后**把语言切回 English**。
   切语言会让页面重载，切完重新走一遍第 3、4 步。
   Plugins 列表那张顺手同拍：**留在列表视图**（不必点进详情页）+ 左侧栏收起态 —— Official / Installed 两组列表，本插件在已安装里可见（卡片上写的是显示名，不是包名）；
   英文 `settings-cards-position_en.png` / 中文 `settings-cards-position.png`。**列表与详情是同一页互斥的两个视图**，配置表单本体不在这张里（它归第 3 步的 `settings-card*.png`）。
   左边栏那张同理：`sidebar-popover.png` / `sidebar-popover_en.png`，**同一次会话里一起拍**，
   两张取景完全一致，否则并排看会发现构图不同（两张都在**自然视口**下、按容器几何裁，见「已知约束」）。
7. **交还**：关掉自己那个标签页 → 把当前会话切回维护者原来那个 → 告诉他可以用了。

拍完把这几张图覆盖进本目录，**文件名不许改** —— 根 README 与 README_en 按名引用。

## 已知约束

- **整屏图不要设置 viewport**：用**自然视口**拍，产出的物理尺寸自然就是 **2449x1223**（CSS 视口 1484x741 × devicePixelRatio 1.65）。
  **不要 `setViewportSize(2449, 1223)`** —— 它会把 dpr 压成 1，界面只剩原生约 60%，正是维护者当场指出的「界面缩小了」。2449x1223 是**物理像素结果**，不是要设的 CSS 视口。
- **devicePixelRatio 本来就是 1.65**：不要去动它，也不要把它压成 1。靠 CDP 改密度那条路被 TabbitDance 拒（`Command rejected`），1.65 是唯一正常值，2x 不可得。
- **`settings-card*.png` 的取法（第三轮实测后改写）**：配置区高 **1293**、自然视口 **741**，它在详情页的滚动容器里。
  直接 `locator.screenshot()` 拍到的是**错位**的一版（上半截空白、下半截被裁）—— 容器裁剪会骗过 `captureBeyondViewport`。
  可靠的顺序是：**先解除祖先裁剪**（配置区到 `documentElement` 逐个 `overflow: visible; max-height: none; height: auto`）→
  量出元素在**文档坐标**里的矩形（`getBoundingClientRect()` + `scrollX/scrollY`）→ `page.screenshot({ fullPage: true })` →
  **在本地按矩形裁**（缩放系数用 `图片高 / 文档高`）。整条路**不碰 viewport**，所以 dpr 不会变。
  取景**含图标 / 标题 / 版本 tag / 描述那一块**（从 `detailHead` 上方一点到保存按钮下方一点），不只是配置表单。
  拍完**必须目检**，并核对产出的物理尺寸约 `1584x2134`（960 CSS 宽 × dpr 1.65 的那一档）。
- **★ 绝对不要 `setViewportSize`**（第三轮实测）：它把 **dpr 打成 1**，而且**把尺寸还原回去也回不来**（实测 1.65 → 1.0）。
  真踩了也别慌：**新开一个页面**（`context.newPage()` + 重新导航），上下文默认的 1.65 会回来；那张被打成 1 倍的图重拍。
  所以「临时调高 viewport 拍长元素」那条老路**不再可行** —— 用上面那条 fullPage + 本地裁。
- **切语言会让当前标签页重载**，切完要重新走一遍 **Plugins → Installed → 本插件**（标题跟着语言变，见第 3 步），并**重新点开四个组头**（默认收起）。
- 四组**默认全部收起**（[BalanceSettingsCard.tsx](../src/client/settings/BalanceSettingsCard.tsx) 的 `DEFAULT_GROUP_OPEN` 全 `false`，本次 UI 改动的结果）：
  进页面只有四个组名，**要拍就得逐个点开组头**；卡片级折叠头已经取消，别去找「展开卡片」那一步。
- 凭据行的徽标只有两态：「已配置密钥。/ 未配置密钥。」（官方「网页搜索」卡片同款）。
- **接口地址那一格默认是空的**：留空就是官方端点，填了才覆盖。图里出现 `https://api.deepseek.com` 说明截的是旧版默认值。
- 凭据输入框**应当是空的、且不置灰**（常态空框）：只读行不写占位符、不降透明度。图里若看到框内有灰字或整格发灰，说明截的是旧产物。
- **Plugins 列表那张同样自己拍**：它拍的是 Plugins 页的**列表视图**（左侧栏**收起态** + Official / Installed 两组列表），**不用点进详情页**；同样要确认图里没有凭据、对话内容、会话标题与工作区名。
- **截图落盘一律给绝对路径**：tabbit 的 nodejs 运行器里 `process.env.TEMP` 是 `undefined`，相对路径会写进浏览器安装目录，事后很难找回来。
- **切完语言要重新走一遍 Plugins → Installed → 本插件**（标题跟着语言变，见第 3 步）：页面重载会退回插件列表。
  现在只剩**分组头** `div[role="button"][aria-expanded]`（**默认 `false`，要拍就得点开**）；卡片级折叠头已经取消。
- **配置区不再需要「落在视口内」这个前提**（第三轮改写）：走的是「解除祖先裁剪 + fullPage + 本地裁」，
  元素比视口高多少都拍得全。**不要**再为了塞进视口去加高 viewport —— 那会把 dpr 打成 1（见上）。
- **`page.screenshot()` 会被浏览器运行时接管**：传了 `path` 也不写你指定的位置，而是落到它的 artifacts 目录，
  并在返回值里给你真实路径 —— 所以**六张都从 artifacts 目录拷回本仓**（第三轮实测：连 `fullPage` 那张也是）。
  `locator.screenshot({ path })` 是原生 Playwright、照写不误，但本轮它拍出来的内容是错的（见上），别再用它拍长元素。
- **全流程都不动 viewport**（第三轮改写）：侧栏那两张、列表那张都在**自然视口**下拍，
  所以不存在「先加高再还原」这一步；谁要是加了高，侧栏的取景与 dpr 都会跟着失真。
- 左边栏那两张**不收侧栏**（全仓唯一例外，见第 4 步）：取景顶端必须压在**浮层 / 底部条目那一叠的上沿**，把工作区名与会话标题整段剔出画面。
  **隐私优先于输入框** —— 宁可不要输入框，也不能露出会话标题；新会话只保证「没有工作文字」，不保证没有侧栏标题。
- **★ 左边界与右边界一律取容器几何，不许手填坐标**（第三轮就是这么翻的车：手填的 390 宽把聊天区的碎片
  —— 一张被裁的卡片、`发送` / `+` 两个按钮 —— 带了进来，英文图里还因此出现了中文 `发送`）：
  - 左 = `[data-slot="sidebar"]` 往上第一个宽 200–560 的祖先的 `x`（实测 0）；
  - 右 = **那个祖先的右边界 与 浮层 `[role=dialog]` 的右边界，取大的那个**。
    **浮层比侧栏宽**（实测侧栏 280、浮层 300 且起点 x=12 ⇒ 右边界 312），按侧栏边缘裁会把浮层的右侧切掉。
    取并集多出来的那 32px **全部被浮层自己盖住**，不是聊天区碎片；实测聊天区内容最近的左边界在 x≈349。
  - 上 = 浮层顶边（实测 450），下 = 视口底（实测 741）⇒ 产出 **515x480**（312 CSS × 1.65）。
  两条边界都要等几何稳定后再量；量完**目检**一遍：右边一列必须是浮层本体或纯背景。

## 验收

- 配置区完整：**插件标题、描述与图标**（插件页按包根 `locale/` 与 `icon.svg` 画）、**版本 tag**（= `package.json` 里那个号）、**四个组名 + 拍摄时点开的四组字段行**、状态徽标、说明行、**唯一的保存按钮**，一样不缺
- 出现折叠头、「放弃」按钮或卡片外框，说明拍的是旧界面
- **两处圆环各按各的判据**：卡片/详情页那个是 `icon.svg`（橙、七成弧、与余额无关）；侧栏条目那条是实时状态环，**红 = `critical`，不是判废**
- **侧栏那两张的右边界之外不许有任何聊天区元素**：图里出现被裁的卡片、`发送` / `+` 这类控件就是判废（第三轮的实际判废理由）
- **整屏 / 宽幅图里左侧栏是收起态**（`sidebar-popover*.png` 两张例外，那里改为裁掉标题段）：看不到会话标题、工作区名或对话内容
- **侧栏是动画中间帧 / 宽度不固定 → 判废重拍**；浮层、折叠区也要等几何稳定，别拿弹出中途的帧凑数（判据见第 4 步的轮询）
- 左边栏那两张：**正常态（币种匹配）下拍** —— 浮层里没有币种不匹配段、条目上没有感叹号标记（若确在不匹配态补拍，那一段必须只有一个「改用 X」按钮）、悬浮气泡不与该浮层同框、浮层标题行右端有 Plugins 图标（**宿主缺 `layout` 服务时它本就不渲染**，缺席是正常的）
- 说明文案与当前 [locales.ts](../src/client/locales.ts) **逐字一致**
- 图里没有真实凭据、真实 Base URL；**余额除外** —— 左边栏那张必然带真实余额，它是真实值、会随时间变，见 [README.md](README.md) 的「其余」
- 文件确实被 [README.md](../README.md) 与 [README_en.md](../README_en.md) 引用（覆盖后跑一次链接校验）
