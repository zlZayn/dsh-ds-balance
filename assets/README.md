...
- `settings-cards-position.png`：**整页设置**的中文实拍（左侧导航 + 插件列表 + 我们那一格），被根 [README.md](../README.md) 引用。
- `settings-cards-position_en.png`：同一画面的英文版，被 [README_en.md](../README_en.md) 引用。

六张都是**从真实界面实拍**的：先拍中文，再临时把界面语言切成 English 拍英文版，拍完切回中文。
...
**整页设置那张**：目的是回答「卡片长在哪」，所以拍整个设置对话框 —— 左侧导航、插件列表、我们那一格都要在。
它必然带进已装插件的名字（官方的与第三方的），那是要交代的上下文，不是隐私问题：设置页里没有对话内容。
