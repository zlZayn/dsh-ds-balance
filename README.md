<p align="center">
  <img src="icon.svg" alt="dsh-ds-balance" width="80" height="80">
</p>
...
| 余额浮层 | 点条目展开：总额、赠送 / 充值拆分、数据新鲜度、手动刷新（带冷却）；**右上角图标跳到本插件的配置格**（宿主没有那条跨插件深链服务时退回 Plugins 列表，悬浮文字同步改口径） | 核对具体数字，以及数据是几分钟前的 |
...
- 插件页里以**本地化名字**出现：中文界面是「DeepSeek 余额」，英文界面是「DeepSeek Balance」——
  卡片与详情页的标题、那句话说明都跟随界面语言，图标就是左边栏那个环（橙色弧环，画的是「用了七成」那一档）；
  详情页仍会把包名 `dsh-ds-balance` 单独写出来。
