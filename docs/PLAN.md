...
> **状态**：后端实现与验证完成，收尾报告在 [final-report.md](final-report.md)。
> **维护者要做的一件事**：重启一次宿主，后端半边才会在主实例上生效（见下文「挂载」）。
...
1. **重启宿主**（唯一还差的一步）。toggle patch 行**不管用**：已实测它只会重新 `apply`
   启动时加载的那个模块，改完重建再 toggle 跑的仍是旧代码。
2. 重启前再确认一次 profile 的 `dsh.profile.bundles` 里没有本插件（双挂载风险）。
3. 重启后自查：`GET /api/v1/healthz` 应回 `store.ok: true`；启动日志里**不该有**任何 `[ds-balance]` 行。
4. 规范向 `dsh-zhihu-search` 靠齐的判定与落地清单 →
   [决策记录](../.agents/notes/2026-09-17-zhihu-search-alignment.md)。
...
2. **mock 旁路的开关**：现在是 URL 参数 / localStorage。若希望改成构建期开关或环境变量，需要改法。
3. **`.salt` 的文件名**：`.salt` 名字很泛，理论上可能与别的工具撞。可改成 `.ds-balance-salt`。
4. **左下角默认币种**：沿用「自动（跟随账户）」。若希望默认固定 CNY，需要改 schema 默认值。
5. **设置卡片折叠状态不持久化**：v1 有意不做。
