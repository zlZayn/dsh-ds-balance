...
- 凭据字段（`apiKey` / `apiKeyRef`）在标签行右侧带状态胶囊：已由启动环境提供 / 已覆盖 / 已配置 / 未配置。
- 「显示币种」是整行左右布局（左文字 + 右选择器胶囊），不是上下结构。
- **连接组照搬官方「模型」卡片的两段式**：外面是只读的凭据状态（disabled 输入框 + 标签行右侧的状态徽章 + 60% 透明度）
  与可编辑的 Base URL；`apiKey` / `apiKeyRef` 收进二级「自定义设置」折叠，默认收起。
