...
- 插件展示面：两份语言文件的键集一致性、字段非空、**门面点名的显示名与 `locale/` 逐字一致**，以及
  **图标与圆环同形**（同几何、弧长 70%、颜色就是 warn 档那个值）—— 三者都由宿主直读，坏了不报错，只能靠断言。
  发布面的覆盖在 [scripts/check-release.mjs](../scripts/check-release.mjs)。
