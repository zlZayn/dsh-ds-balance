...
- **例外二：`contract-live-*.test.ts`** 打真实上游；凭据先取 `DSH_CI_API_KEY`（契约巡检的专用 key），缺了回落 `DEEPSEEK_API_KEY`（本机日常那把）。只由 `vitest.contract.config.ts` 收集。
  解析判据在 [contract-key.ts](contract-key.ts) —— **别把它写回测试文件顶部**：那里是模块加载期 throw，改了就没法单测。
